...
{
    'use strict'; $.HSCore = {
        init: function () { $(document).ready(function (e) { $('[data-toggle="tooltip"]').tooltip({ trigger: 'hover' }); $('[data-toggle="popover"]').popover({ trigger: 'hover' }); $.HSCore.helpers.darkMode(); $.HSCore.helpers.detectIE(); $.HSCore.helpers.bootstrapNavOptions.init(); }); }, components: {}, helpers: {
            Math: { getRandomValueFromRange: function (startPoint, endPoint, fixed) { var fixedInner = fixed ? fixed : false; Math.random(); return fixedInner ? (Math.random() * (endPoint - startPoint) + startPoint) : (Math.floor(Math.random() * (endPoint - startPoint + 1)) + startPoint); } }, darkMode: function ()
            {
                $('#darkModaBtn').on('click', function ()
                {
                    $('body').toggleClass('dark-mode'); if ($('body').hasClass('dark-mode')) { writeCookie("displaymode", "dark"); $('#darkModaBtnIcon').toggleClass('fa-moon fa-sun'); $('#logo-header').attr('src',); window.mode = 'dark'; $("iframe").each(function () { var src = $(this).contents().find('body'); src.addClass('dark-mode'); src.attr('style', 'background: #212121 !important;'); var subIframe = $(this).contents().find('iframe'); if (subIframe.length > 0) { var subIframeBody = $(subIframe).contents().find('body'); var style = 'background: #212121 !important;'; subIframeBody.addClass('dark-mode'); subIframeBody.attr('style', style); } }); $('#dappBody').css('background-image', 'url("/assets/svg/illustrations/dapp-bg-dark.svg")'); } else { writeCookie("displaymode", "normal"); $('#darkModaBtnIcon').toggleClass('fa-sun fa-moon'); $('#logo-header').attr('src', '/images/logoRoc.png'); window.mode = 'normal'; $("iframe").each(function () { var src = $(this).contents().find('body'); src.attr('class', ''); src.removeAttr("style"); var subIframe = $(this).contents().find('iframe'); if (subIframe.length > 0) { var subIframeBody = $(subIframe).contents().find('body'); subIframeBody.attr('class', ''); subIframeBody.removeAttr("style"); } }); $('#dappBody').css('background-image', 'url("/assets/svg/illustrations/dapp-bg.svg")'); }
                    if (typeof loaddisqus !== 'undefined' && $.isFunction(loaddisqus)) { if (disqusloaded === true) { disqusloaded = false; loaddisqus(); } }
